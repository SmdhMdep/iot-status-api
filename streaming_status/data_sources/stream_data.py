...
def get_stream_preview(topic: str) -> tuple[str, datetime | None] | None:
    ...
    try:
        _, _, org_name, project_name, resource_name = topic.removeprefix("$aws/").removeprefix("rules/").split("/")
    except ValueError:
    ...
    with BytesIO() as memory_file:
        try:
            ...
            return "\n".join(line.decode() for _, line in zip(range(_PREVIEW_MAX_LINES), memory_file)), last_modified
...
def _find_storage_path(package: dict, name: str) -> tuple[str, datetime | None] | None:
    for resource in package["resources"]:
        if resource["name"] == name:
            ...
            return (resource["cloud_storage_key"], last_modified if "cloud_storage_key" in resource else None)
    return None
...
def _find_package(org_name: str, project_name: str):
    ...
    if response.status_code == requests.codes["forbidden"]:
        ...
        logger.error(
            "got forbidden status code from MDEP API trying to request cloudstorage_package_show with name %s",
            project_name,
        )
        return None
