...
def list_schemas(
    provider: str | None, page: str | None = None, page_size: int | None = None
    ...
    next_page_encoded = base64.encodebytes(json.dumps(next_page).encode()).decode() if next_page else None

    return next_page_encoded, sorted(
        result["Items"], key=lambda item: (item["jwtGroup"], item["title"], -item["version"])
    )
...
def get_schema_by_hash(provider: str, json_schema: str) -> dict | None:
    ...
    items = schemas_table.query(
        IndexName="schemaHash-index", KeyConditionExpression=conditions.Key("schemaHash").eq(schema_hash)
    ).get("Items")
