...
def groups(name_like: str | None, page: int, page_size: int, *, token) -> tuple[int | None, list[str]]:
    ...
    groups = _unwrap(
        requests.get(
            f"{config.keycloak_admin_api_url}/groups", headers={"Authorization": f"Bearer {token}"}, params=params
        )
    )
    return (page + 1 if len(groups) >= page_size else None, [group["name"] for group in groups])
